...
<template>
    <div class=" rounded  d-flex flex-column mt-3 mb-3 bg-white shadow ps-3 pe-3">
        <div class="p-4 d-flex ">
            ...
            <div class="dropdown d-flex align-items-center" data-bs-toggle="tooltip" title="Choose Display">
                <button class="btn active rounded-circle " type="button" data-bs-toggle="dropdown" aria-expanded="false">
                    <span class="mdi mdi-dots-vertical"></span>
                </button>
                <ul class="dropdown-menu">
                    <li><a @click="display = item" v-for="item in displays" :key="item.name" class="dropdown-item"
                            href="#">{{ item.name }} ({{ item.details }})</a></li>
                </ul>
            </div>
        </div>
        <div ref="dis" data-bs-toggle="tooltip" title="Scroll to resize" :class="cssClass" :style="{ scale: display.scale }"
            @mousewheel="handleScroll">
            <div id="screen"
                :style="{ width: display.width + 'px', height: display.height + 'px', backgroundColor: display.backgroundColor, color: display.color }"
                class="shadow position-relative">
                <label v-for="item in labels" :key="item.id" :style="{
                    left: item.left + 'px',
                    top: item.top + 'px',
